...
<form novalidate action="{{ route('vacancies-admin') }}" method="GET" onsubmit="$('.page-loader').show()">
    <div class="row">
        <div class="col-md-4">
            <h1 class="page-heading rounded">Vacancies <span class="badge rounded-pill bg-warning text-white ms-3">{{$data->total()}}</span></h1>
        </div>
        <div class="col-md-8">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="position" id="position">
                                ...
                                <!-- @foreach ($positions as $obj)
                                    <option {{$positionFilter == $obj->id ? 'selected' : ''}} value="{{$obj->id}}">{{$obj->name}}</option>
                                @endforeach -->
                            </select>
                        </div>
                    </div>
                ...
                <tbody id="tbody-data">
                    @foreach ($data as $obj)
                        <tr>
                            ...
                            <td>
                                <div class="d-flex">
                                    <a href="{{ route('show-vacancy', $obj->code) }}"  title="View Vacancy" class="btn btn-sm btn-success btn-action"><i class="fas fa-eye"></i></a>
                                    <a target="_blank" href="{{ route('show-application', $obj->code) }}?status=All"  title="View Applicants" class="btn btn-sm btn-success btn-action mx-1"><i class="fas fa-users"></i></a>
                                    <form novalidate action="{{ route('destroy-vacancy',$obj->id) }}" class="confirm-action" title="Delete" method="POST">
                                        @csrf
                                        @method('DELETE')
